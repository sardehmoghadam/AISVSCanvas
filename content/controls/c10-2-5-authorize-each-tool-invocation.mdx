...
standardVersion: "1.0"
controlId: C10.2.5
canonicalId: v1.0-C10.2.5
slug: c10-2-5-authorize-each-tool-invocation
title: "Authorize each tool invocation"
summary: "MCP servers enforce access control on every tool invocation, validating that the user's access token authorizes both the requested tool and the specific argu..."
chapter:
  id: C10
...
relatedControls:
  - id: C10.2.6
    title: "Remove session artifacts on termination"
    href: /controls/c10-2-6-remove-session-artifacts-on-termination/
  - id: C10.2.4
...
MCP servers enforce access control on every tool invocation, validating that the user's access token authorizes both the requested tool and the specific argu... This is a Level 2 requirement of OWASP AISVS 1.0 (Model Context Protocol (MCP) Security).
...
> Verify that MCP servers enforce access control on every tool invocation, validating that the user's access token authorizes both the requested tool and the specific argument values supplied.
