...
standardVersion: "1.0"
controlId: C10.2.6
canonicalId: v1.0-C10.2.6
slug: c10-2-6-remove-session-artifacts-on-termination
title: "Remove session artifacts on termination"
summary: "MCP servers ensure all session artifacts are removed when a session terminates."
chapter:
  id: C10
...
MCP servers ensure all session artifacts are removed when a session terminates. This is a Level 2 requirement of OWASP AISVS 1.0 (Model Context Protocol (MCP) Security).
...
> Verify that MCP servers ensure all session artifacts are removed when a session terminates.
